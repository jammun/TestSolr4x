...
public class KoreanAnalyzer extends StopwordAnalyzerBase {
  ...
  static {
    List<String> stopWords = Arrays.asList(new String[] { "a", "an", "and", "are", "as", "at", "be", "but", "by", 
        ...
        "their", "then", "there", "these", "they", "this", "to", "was", "will", "with","이", "그","저","것","수","등","들","및","에서","그리고","그래서","또","또는"}
    );
  ...
  public KoreanAnalyzer(Version matchVersion, CharArraySet stopWords) {
    ...
    replaceInvalidAcronym = matchVersion.onOrAfter(Version.LUCENE_44);     
  }  
  ...
  protected TokenStreamComponents createComponents(final String fieldName, final Reader reader) {
    ...
    TokenStream tok = new KoreanFilter(src, bigrammable, hasOrigin, exactMatch, originCNoun, isPositionInc, queryMode, doDecompound);
  ...
  /**
   * determin whether this is query mode.
   * if true, auto spacing is processed for the all words which don't exist in dictionary. 
   */
  ...
  /**
   * determin whether compound nouns should be decompounded.
   * if you use phrase query or proximate query, this value should be set to false
   */
  public void setDoDecompound(boolean decompound) {
    doDecompound = decompound;
  }
