...
//        break;  
  ...
  private boolean analyze4Word(String input,List outputs, boolean isFirst) throws MorphException {
    ...
    if(!exactMach&&existSome(entries1)) {
      outputs.addAll(Arrays.asList(entries1));  
      return true;
    }
  ...
  private boolean analyze5Word(String input,List outputs, boolean isFirst) throws MorphException {
    ...
    if(!exactMach&&existSome(entries2)) {  
      outputs.addAll(Arrays.asList(entries2));  
      return true;
    }
    
    boolean is = false;
    if(!exactMach&&existSome(entries3)) {
      outputs.addAll(Arrays.asList(entries3));
      return true;
    } 
    
    if(!exactMach&&existSome(entries4)) {
      outputs.addAll(Arrays.asList(entries4));
      is = true;
    }   
  ...
  private boolean existAll(CompoundEntry[] entries) {
	  
	  if(entries==null) return false;	  
	  for(CompoundEntry entry : entries) {		  
		  if(!entry.isExist()) return false;		  
	  }	  
	  return true;
  }
  
  private boolean existSome(CompoundEntry[] entries) {
	  
	  if(entries==null) return false;	  
	  for(CompoundEntry entry : entries) {
		  if(entry.getWord().length()==1) continue;
		  if(entry.isExist()) return true;
	  }	  
	  return false;
  }
  ...
  private CompoundEntry[] analysisBySplited(int[] units, String input, boolean isFirst) throws MorphException {
    ...
    for(int i=0;i<units.length;i++) {
      ...
      entries.addAll(analyzeSingle(str)); // CompoundEntry 로 변환

      pos += units[i];
      prev = str;
    }
    
    return entries.toArray(new CompoundEntry[0]);
   ...
   */
  private List<CompoundEntry> analyzeSingle(String input) throws MorphException {
    ...
    List<CompoundEntry> cEntryies = new ArrayList<CompoundEntry>();
    
    if(input.length()==1) {
    	cEntryies.add(new CompoundEntry(input, 0, true,pos));
    	return cEntryies;
    }
    ...
    cEntryies.add(new CompoundEntry(input, 0, score==AnalysisOutput.SCORE_CORRECT,pos));
